...
PHP_FUNCTION(__call)
{
	...
	if(zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "sa|ss",
		...
		php_error(E_ERROR, "Invalid arguments to SoapObject->call");
	...
	get_http_soap_response(getThis(), NULL, function, &ret_params, &num_params);
	...
	else
	{
		...
		if(sdl != NULL)
		{
			...
			if(fn != NULL)
			{
				...
				get_http_soap_response(getThis(), fn, NULL, &ret_params, &num_params);
				if(num_params > 0)
				{
					*return_value = *ret_params[0];
					zval_add_ref(&return_value);
